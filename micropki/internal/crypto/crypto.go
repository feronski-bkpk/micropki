...
// Поддерживаемые типы: RSA-4096 и ECC P-384
func GenerateKeyPair(keyType string, keySize int) (*KeyPair, error) {
	switch keyType {
	case "rsa":
		if keySize != 4096 {
			return nil, fmt.Errorf("размер RSA ключа должен быть 4096 бит")
		}
	...
	case "ecc":
		if keySize != 384 {
			return nil, fmt.Errorf("размер ECC ключа должен быть 384 бит (кривая P-384)")
		}
		
		// Генерируем ECDSA ключ на кривой P-384
...
// Это современный и безопасный метод шифрования (замена устаревшему PEM encryption)
func encryptPrivateKey(keyBytes []byte, passphrase []byte) ([]byte, error) {
	...
	// Используем ASN.1 для совместимости с различными инструментами
...
// Использует современное AES-256-GCM шифрование с PBKDF2
